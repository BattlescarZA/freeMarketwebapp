# VibeFinance - Supabase Setup Guide

## 🎯 Overview

This guide covers the complete Supabase setup for VibeFinance, including:
- Project creation
- Database schema
...
Copy and paste each section into the SQL Editor and run them in order.
...
#### Assets Policies
```sql
-- Assets: Read-only for all authenticated users
...
```sql
-- Create view for easy portfolio value calculation
...
BEGIN
  RETURN QUERY
  SELECT
    COALESCE(SUM(CASE WHEN transaction_type = 'buy' THEN quantity * price ELSE 0 END), 0) AS total_buys,
    COALESCE(SUM(CASE WHEN transaction_type = 'sell' THEN quantity * price ELSE 0 END), 0) AS total_sells,
...
-- Expected: assets, holdings, portfolios, price_alerts, profiles, transactions, watchlist
...
### Trigger not firing on transactions
- Check trigger exists: `\d public.transactions`
- Check function exists and is valid
- Test manually by inserting transaction
...
**Status:** Supabase setup guide complete
